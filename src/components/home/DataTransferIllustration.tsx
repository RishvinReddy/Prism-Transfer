...
// Dense 12x12 Mini QR Code component with hollow-center finder patterns
function MiniQR() {
  const [grid, setGrid] = React.useState<boolean[]>([]);
  
  React.useEffect(() => {
    const interval = setInterval(() => {
      setGrid(Array.from({ length: 144 }, () => Math.random() > 0.45)); // 12x12 = 144 modules
    }, 120);
  ...
  return (
    <div className="grid grid-cols-12 grid-rows-12 gap-[1px] p-1.5 bg-white rounded-lg border border-zinc-200 w-24 h-24 shadow-inner relative z-10">
      {grid.map((val, idx) => {
        const x = idx % 12;
        const y = Math.floor(idx / 12);
        
        // Exact 4x4 finder patterns with hollow 2x2 centers
        const isTLFinder = x < 4 && y < 4 && !((x === 1 || x === 2) && (y === 1 || y === 2));
        const isTRFinder = x >= 8 && y < 4 && !((x === 9 || x === 10) && (y === 1 || y === 2));
        const isBLFinder = x < 4 && y >= 8 && !((x === 1 || x === 2) && (y === 9 || y === 10));
        
        const isFinder = isTLFinder || isTRFinder || isBLFinder;
        const inFinderZone = (x < 4 && y < 4) || (x >= 8 && y < 4) || (x < 4 && y >= 8);

        return (
          <div 
            key={idx} 
            className={cn(
              "w-full h-full rounded-[0.5px] transition-colors duration-75",
              isFinder || (val && !inFinderZone) ? "bg-black" : "bg-white"
            )}
          />
        );
      })}
...
export function DataTransferIllustration() {
  ...
  return (
    <div className="relative w-full aspect-[4/3] max-w-[480px] lg:max-w-[550px] overflow-hidden flex items-center justify-center select-none pointer-events-none">
      ...
      {/* 3D Perspective Context Container */}
      <div 
      ...
      >
        ...
        {/* Foliage Left */}
        <div className="absolute top-[50%] left-[5%] rotate-[-20deg] flex flex-wrap gap-1.5 w-[100px] z-10" style={{ transform: "translateZ(20px)" }}>
        ...
        {/* Foliage Right */}
        <div className="absolute top-[55%] right-[5%] rotate-[20deg] flex flex-wrap gap-1.5 w-[100px] z-10" style={{ transform: "translateZ(20px)" }}>
          <div className="w-8 h-8 bg-emerald-600/30 border border-emerald-400/20 rounded-tr-[32px] rounded-bl-[32px]" />
          <div className="w-8 h-8 bg-emerald-800/20 border border-emerald-500/20 rounded-tr-[32px] rounded-bl-[32px]" />
        </div>
        ...
        >
          <div className="w-[140px] h-[290px] bg-zinc-950 border border-zinc-850 rounded-[20px] relative overflow-hidden flex flex-col justify-between py-4 items-center">
            ...
            {/* Offline Status Bar */}
            <div className="w-full px-4 flex justify-between items-center text-[7px] text-zinc-500 font-mono tracking-tight pt-1 relative z-10">
              <span>9:41 AM</span>
            ...
            <div className="relative w-[140px] h-[85px] z-10">
        ...
        >
          {/* Screen */}
          <div className="w-[260px] h-[170px] bg-zinc-900 border-[12px] border-zinc-800 border-b-[20px] rounded-t-[10px] relative overflow-hidden flex flex-col justify-end shadow-[15px_15px_40px_rgba(0,0,0,0.5)]">
            <div className="w-full h-full bg-zinc-950 flex flex-col justify-between p-2.5 select-none border border-zinc-850 relative">
              
              {/* Gloss reflection overlay */}
              <div className="absolute inset-0 bg-gradient-to-tr from-transparent via-white/5 to-white/10 pointer-events-none z-20 transform rotate-12 scale-150 translate-x-[-15%]" />
              
              {/* Receiver Header */}
              <div className="flex justify-between items-center text-[7px] text-zinc-500 font-mono tracking-wider border-b border-zinc-900 pb-1 relative z-10">
                <span>RECEIVER CONSOLE</span>
                <span className="w-1.5 h-1.5 rounded-full bg-cyan-500 animate-pulse" />
              </div>
              
              {/* Progress HUD */}
              <div className="flex items-center space-x-2 my-1.5 relative z-10">
                <div className="relative w-8 h-8 flex items-center justify-center flex-shrink-0">
                  ...
                  <span className="absolute text-[7px] font-bold text-white font-mono">65%</span>
                </div>
              ...
              {/* Folder in Laptop */}
              <div className="relative w-full h-[55px] z-10 mt-1">
